...
bildungLayer.on('clusterclick', function(cluster) {
  ...
  /*
	...
	for (var i = 0; i < markers.length; i++) {
		var m = markers[i];
		var key = m.feature.properties.type_abbr
		var count = m.feature.properties.Total
		total = total + count;
  ...
  // Compute the position of each group on the pie:
	...
	svg
		...
		.attr('fill', function(d){ return(colorsBildungTypes[d.data.key]) })
		.attr("stroke", "black")
